    ...
    try {
      const token = await AsyncStorage.getItem('token');
        ...
        const response = await axios.post(
          'http://192.168.1.15:8080/api/v1/auth/surplus',
          { plates: plate, kwPerPlate: kilowat, pricePerUnit: price, longi, lati, availabilityStart: start, availabilityEnd: end, unit, location: loc },
          {
            headers: {
              'authorization': token,
  ...
  return (
    ...
    <View style={styles.inRad}>
        ...
        <View style={styles.wdt1}>
          <View style={styles.inRad}>
            <TextInput
              placeholder='Starting bhj bhj Date'
              color='black'
              keyboardType='numeric'
              style={styles.inpt}
