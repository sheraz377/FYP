    ...
    const adb = async ()=>{
        try {
            ...
            const response = await axios.post(
              'http://192.168.1.15:8080/api/v1/auth/showSurplus',
              {}, // Request body if any
              {
                headers: {
                  'authorization': token,
...
useEffect(()=>{
    ...
    return (
        <>
            ...
            renderItem={({item})=>(
                <Card2 unit={item.unit} id={item._id}location={item.location} name={item.userName} start={(item.availabilityStart).split('T')[0]} end={(item.availabilityEnd).split('T')[0]}price={item.pricePerUnit}></Card2>
            )}
            ></FlatList>
...
const Combine1 = () => {
    ...
    const adb = async ()=>{
        try {
            ...
            const response = await axios.post(
              'http://192.168.1.10:8080/api/v1/auth/showEv',
              {}, // Request body if any
              {
                headers: {
                  'authorization': token,
